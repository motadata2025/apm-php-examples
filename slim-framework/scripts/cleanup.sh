...
cleanup_apache() {
    ...
    # Disable and remove all slim-framework sites
    for site in /etc/apache2/sites-available/slim-framework*.conf; do
        if [ -f "$site" ]; then
    ...
    sudo rm -f /etc/apache2/sites-available/slim-framework*.conf
...
cleanup_application_copies() {
    ...
    local apache_dirs=("/var/www/slim-framework" "/var/www/slim-framework-*")
    ...
    if sudo find /var/www -name "*slim-framework*" -type d 2>/dev/null | grep -q .; then
        echo -e "  ${BLUE}Removing additional application copies...${NC}"
        sudo find /var/www -name "*slim-framework*" -type d -exec rm -rf {} + 2>/dev/null || true
...
cleanup_logs() {
    ...
    if [ -f "/var/log/apache2/slim-framework_error.log" ] || [ -f "/var/log/apache2/slim-framework_access.log" ]; then
        ...
        if [[ "$remove_apache_logs" =~ ^[Yy]$ ]]; then
            ...
            sudo rm -f /var/log/apache2/slim-framework*.log
