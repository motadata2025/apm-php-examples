...
stop_all_php_fpm() {
    ...
    for version in 7.4 8.0 8.1 8.2 8.3 8.4; do
        if systemctl is-active "php${version}-fpm" >/dev/null 2>&1; then
            echo -e "    ${YELLOW}Stopping PHP-FPM ${version}...${NC}"
            if sudo systemctl stop "php${version}-fpm" 2>/dev/null; then
                echo -e "    ${GREEN}✅ PHP-FPM ${version} stopped${NC}"
                stopped_any=true
            else
                echo -e "    ${RED}❌ Failed to stop PHP-FPM ${version}${NC}"
            fi
        fi
    done
...
# Function to start specific PHP-FPM version
start_php_fpm_version() {
    local version=$1
    echo -e "  ${BLUE}Starting PHP-FPM ${version}...${NC}"

    if sudo systemctl start "php${version}-fpm" 2>/dev/null; then
        echo -e "  ${GREEN}✅ PHP-FPM ${version} started${NC}"
        return 0
    else
        echo -e "  ${RED}❌ Failed to start PHP-FPM ${version}${NC}"
        return 1
    fi
}
...
get_deployment_options() {
    ...
    echo -e "   ${BLUE}Command: php${php_version} -S 0.0.0.0:8000${NC}"
                ...
                "available_disabled")
                    ...
                    if [[ "$enable_mod_php" =~ ^[Yy]$ ]]; then
                        ...
                        for fpm_version in 7.4 8.0 8.1 8.2 8.3 8.4; do
                            if systemctl is-active "php${fmp_version}-fpm" >/dev/null 2>&1; then
                                echo -e "    ${YELLOW}Stopping PHP-FPM ${fmp_version}...${NC}"
                                sudo systemctl stop "php${fmp_version}-fpm" 2>/dev/null || true
                            fi
                        done
                        echo -e "  ${GREEN}✅ All PHP-FPM services stopped${NC}"
