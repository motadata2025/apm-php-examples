...
cleanup_laravel_caches() {
    echo -e "  ${BLUE}🧹 Cleaning up Laravel caches...${NC}"
    ...
    if [[ -d "vendor" ]] && command -v php >/dev/null 2>&1; then
        ...
        echo -e "  ${YELLOW}⚠️  Laravel not available, skipping cache cleanup${NC}"
    fi
}
...
stop_application() {
    ...
    # Laravel-specific: Clean up caches
    cleanup_laravel_caches
