...
handle_dependency_management() {
    ...
    if [[ -f "composer.json" ]]; then
        ...
        # Laravel-specific: Handle APP_KEY generation
        handle_laravel_app_key
    else
        log_warning "No composer.json found - skipping dependency management"
    fi
}

# Laravel-specific APP_KEY handling
handle_laravel_app_key() {
    echo ""
    echo -e "${BLUE}🔑 Laravel APP_KEY Configuration${NC}"
    echo "=================================="
    ...
    if [[ -z "$current_key" ]] || [[ "$current_key" == "base64:\$(openssl rand -base64 32)" ]]; then
        ...
        log_success "APP_KEY generated and saved to .env"
        echo -e "${GREEN}🔑 APP_KEY: $new_key${NC}"
    else
        log_success "APP_KEY already configured"
        echo -e "${GREEN}🔑 Current APP_KEY: ${current_key:0:20}...${NC}"
    fi

    # Clear Laravel caches to ensure new key is picked up
