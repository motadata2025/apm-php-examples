...
# CodeIgniter-specific cache clearing on shutdown
cleanup_codeigniter_caches() {
    echo -e "  ${BLUE}🧹 Cleaning up CodeIgniter caches...${NC}"
    ...
    if [[ -d "writable/uploads" ]]; then
        find writable/uploads -name "tmp_*" -mtime +1 -delete 2>/dev/null || true
...
stop_application() {
    ...
    # CodeIgniter-specific: Clean up caches on shutdown
    cleanup_codeigniter_caches
