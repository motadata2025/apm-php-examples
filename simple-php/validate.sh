#!/bin/bash
# validate.sh - Simple PHP Application Validator Wrapper
...
if [[ ! -f "$VALIDATOR_PHP" ]]; then
    ...
    echo '{"app":"simple-php","success":false,"error":"validator_missing","timestamp":'$(date +%s)'}'
    exit 1
fi
...
if timeout 120 php "$VALIDATOR_PHP"; then
    ...
    if [[ $exit_code -eq 124 ]]; then
        echo '{"app":"simple-php","success":false,"error":"timeout","timeout_seconds":120,"timestamp":'$(date +%s)'}'
    fi
    exit $exit_code
fi
