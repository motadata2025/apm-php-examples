...
# Apache mod_php Deployment Script
...
# Function to create Apache virtual host configuration for mod_php
create_apache_vhost() {
    echo -e "\n${PURPLE}🌐 Creating Apache Virtual Host Configuration (mod_php)${NC}"
    ...
    # Create virtual host configuration for mod_php
    sudo tee "$vhost_file" > /dev/null << EOF
<VirtualHost *:$APP_PORT>
    ServerName $server_name
    DocumentRoot $document_root
    
    # mod_php Configuration - PHP is handled by Apache module
    # No need for proxy configuration like PHP-FPM
    ...
    <Directory $document_root>
        ...
        # Ensure PHP files are processed by mod_php
        <Files "*.php">
...
enable_apache_modules() {
    ...
    if sudo a2enmod "php${PHP_VERSION}" 2>/dev/null; then
        echo -e "  ${GREEN}✅ Module php${PHP_VERSION} enabled${NC}"
    else
        echo -e "  ${YELLOW}⚠️  Module php${PHP_VERSION} already enabled${NC}"
    fi
}
...
# Function to verify mod_php is working
verify_mod_php() {
    echo -e "\n${PURPLE}🐘 Verifying mod_php${NC}"
    
    if sudo a2enmod -q "php${PHP_VERSION}" 2>/dev/null; then
        echo -e "  ${GREEN}✅ mod_php ${PHP_VERSION} is enabled${NC}"
    else
        echo -e "  ${RED}❌ mod_php ${PHP_VERSION} is not enabled${NC}"
        return 1
    fi
}
...
show_access_info() {
    echo -e "\n${PURPLE}🌐 Apache mod_php Deployment Complete${NC}"
    echo -e "========================================"
    ...
    echo -e "  ${GREEN}Web Server: Apache with mod_php${NC}"
...
# Main execution
main() {
    echo -e "${BLUE}🚀 Apache mod_php Deployment${NC}"
    echo -e "============================="
