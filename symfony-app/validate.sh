#!/bin/bash
# validate.sh - Symfony Application Validator Wrapper
...
if [[ ! -f "$ENV_FILE" ]]; then
    if [[ -f "$ENV_EXAMPLE" ]]; then
        ...
        echo '{"app":"symfony-app","success":false,"error":"env_file_missing","timestamp":'$(date +%s)'}'
        exit 1
    fi
fi
...
if [[ ! -f "$VALIDATOR_PHP" ]]; then
    ...
    echo '{"app":"symfony-app","success":false,"error":"validator_missing","timestamp":'$(date +%s)'}'
    exit 1
fi
...
if timeout 120 php "$VALIDATOR_PHP"; then
    ...
    if [[ $exit_code -eq 124 ]]; then
        echo '{"app":"symfony-app","success":false,"error":"timeout","timeout_seconds":120,"timestamp":'$(date +%s)'}'
    fi
    exit $exit_code
fi
