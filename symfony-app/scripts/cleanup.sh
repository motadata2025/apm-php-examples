...
# APM PHP Examples - Symfony Application
...
cleanup_apache() {
    ...
    # Disable and remove all symfony-app sites
    for site in /etc/apache2/sites-available/symfony-app*.conf; do
        if [ -f "$site" ]; then
    ...
    sudo rm -f /etc/apache2/sites-available/symfony-app*.conf
...
cleanup_application_copies() {
    ...
    local apache_dirs=("/var/www/symfony-app" "/var/www/symfony-app-*")
    ...
    if sudo find /var/www -name "*symfony-app*" -type d 2>/dev/null | grep -q .; then
        echo -e "  ${BLUE}Removing additional application copies...${NC}"
        sudo find /var/www -name "*symfony-app*" -type d -exec rm -rf {} + 2>/dev/null || true
...
cleanup_logs() {
    ...
    if [ -f "/var/log/apache2/symfony-app_error.log" ] || [ -f "/var/log/apache2/symfony-app_access.log" ]; then
        ...
        if [[ "$remove_apache_logs" =~ ^[Yy]$ ]]; then
            ...
            sudo rm -f /var/log/apache2/symfony-app*.log
