...
main() {
    ...
    if [[ ! -f "$RUNTIME_CONFIG" ]] && [[ -f "config/app.env" || -f ".env" ]]; then
        ...
        # Run migration script without failing on exit code
        set +e
        ...
        if [[ $migration_result -ne 0 ]]; then
            # Check if runtime config was actually created despite exit code
            if [[ -f "$RUNTIME_CONFIG" ]]; then
                log_success "Configuration migration completed (runtime config exists)"
            else
